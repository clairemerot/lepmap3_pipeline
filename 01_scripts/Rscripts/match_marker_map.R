...
for (j in 1 : NB_CHR)
{
order<-read.table(paste0("07_order_LG/order_",MISS,"_",MIN_COV,"_",i,"_LOD_",LOD,".LG",j,".",(REFINE_STEPS + 1),".txt"), stringsAsFactors=F) [,1:3]
head(order)
...
write.table(contig_map[2: dim(contig_map)[1],], paste0("08_analyze_maps/01_maps/contig_order_",MISS,"_",MIN_COV,"_",i,"_LOD_",LOD,".LGall.refined",(REFINE_STEPS + 1),".txt") , row.names=F,quote=F, sep="\t")
write.table(contig_map_filtered[2: dim(contig_map_filtered)[1],], paste0("08_analyze_maps/01_maps/contig_order_",MISS,"_",MIN_COV,"_",i,"_LOD_",LOD,".LGall.refined",(REFINE_STEPS + 1),".f.txt") , row.names=F,quote=F, sep="\t")

jpeg(paste0("08_analyze_maps/02_plot/contig_order_",MISS,"_",MIN_COV,"_",i,"_LOD_",LOD,".LGall.refined",(REFINE_STEPS + 1),".jpeg"))
plot(contig_map[2:dim(contig_map)[1],4], ylab="female_pos")
dev.off()
	 
jpeg(paste0("08_analyze_maps/02_plot/contig_order_",MISS,"_",MIN_COV,"_",i,"_LOD_",LOD,".LGall.refined",(REFINE_STEPS + 1),".f.jpeg"))
